...
Use this template to generate a prompt for any `board-task-in-column-reorder` without hardcoding spec names.

---

You are implementing OpenSpec change: `board-task-in-column-reorder` in Fast Forward mode.
...
- Change path: `openspec/changes/board-task-in-column-reorder/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/board-task-in-column-reorder/tasks.md`.
...
- Validation log: commands + results (including `openspec validate board-task-in-column-reorder` and test runs)
