...
Use this template to generate a prompt for any `app-data-persistence` without hardcoding spec names.

---

You are implementing OpenSpec change: `app-data-persistence` in Fast Forward mode.
...
- Change path: `openspec/changes/app-data-persistence/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/app-data-persistence/tasks.md`.
...
- Validation log: commands + results (including `openspec validate app-data-persistence` and test runs)
