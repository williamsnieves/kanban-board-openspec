...
Use this template to generate a prompt for any `board-card-collaboration-features` without hardcoding spec names.

---

You are implementing OpenSpec change: `board-card-collaboration-features` in Fast Forward mode.
...
- Change path: `openspec/changes/board-card-collaboration-features/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/board-card-collaboration-features/tasks.md`.
...
- Validation log: commands + results (including `openspec validate board-card-collaboration-features` and test runs)
