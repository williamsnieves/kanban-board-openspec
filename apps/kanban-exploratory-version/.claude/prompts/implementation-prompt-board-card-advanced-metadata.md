...
Use this template to generate a prompt for any `board-card-advanced-metadata` without hardcoding spec names.

---

You are implementing OpenSpec change: `board-card-advanced-metadata` in Fast Forward mode.
...
- Change path: `openspec/changes/board-card-advanced-metadata/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/board-card-advanced-metadata/tasks.md`.
...
- Validation log: commands + results (including `openspec validate board-card-advanced-metadata` and test runs)
