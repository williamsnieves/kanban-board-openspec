...
Use this template to generate a prompt for any `board-management-flow` without hardcoding spec names.

---

You are implementing OpenSpec change: `board-management-flow` in Fast Forward mode.
...
- Change path: `openspec/changes/board-management-flow/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/board-management-flow/tasks.md`.
...
- Validation log: commands + results (including `openspec validate board-management-flow` and test runs)
