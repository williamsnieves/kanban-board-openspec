...
Use this template to generate a prompt for any `<change_slug>` without hardcoding spec names.

---

You are implementing OpenSpec change: `<change_slug>` in Fast Forward mode.
...
- Change path: `openspec/changes/<change_slug>/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/<change_slug>/tasks.md`.
...
- Validation log: commands + results (including `openspec validate <change_slug>` and test runs)
