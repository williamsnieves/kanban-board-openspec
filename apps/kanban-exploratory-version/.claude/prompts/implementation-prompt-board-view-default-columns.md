...
Use this template to generate a prompt for any `board-view-default-columns` without hardcoding spec names.

---

You are implementing OpenSpec change: `board-view-default-columns` in Fast Forward mode.
...
- Change path: `openspec/changes/board-view-default-columns/`
- Follow `AGENTS.md` once created and project conventions.

Execution protocol (strict):

- Work one unchecked task at a time from `openspec/changes/board-view-default-columns/tasks.md`.
...
- Validation log: commands + results (including `openspec validate board-view-default-columns` and test runs)
